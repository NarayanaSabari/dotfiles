...
- no-mistakes TUI split: when you start a no-mistakes run and `HERDR_ENV=1`, first split a herdr pane to the right (`herdr pane split "$HERDR_PANE" --direction right --no-focus`), then run `no-mistakes attach` in that new pane to show the live TUI beside pi, and drive `no-mistakes axi run` from the current pane. Parse the new pane id from `result.pane.pane_id`. Skip the split entirely when `HERDR_ENV` is not `1` - never control herdr from outside it.
