...
- no-mistakes TUI split: when you start a no-mistakes run and `HERDR_ENV=1`, first split the current pane to the right with `herdr pane split --current --direction right --no-focus` (the `--current` flag targets your own pane; do not use a `$HERDR_PANE` env var, it does not exist). Read the new pane id from `result.pane.pane_id`, run `no-mistakes attach` in it to show the live TUI beside pi, then drive `no-mistakes axi run` from the current pane. Skip the split entirely when `HERDR_ENV` is not `1` - never control herdr from outside it, because `herdr pane current`/`--current` then falls back to whatever pane is focused (a different session) and the split would land in the wrong place.
