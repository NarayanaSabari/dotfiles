...
- `~/.jcode/skills/` and `~/.claude/skills/` both symlink into `~/.agents/mattpocock-skills/skills/{engineering,productivity}/`, plus standalone skills cloned under `~/.agents/<name>` (currently `archify`, for interactive HTML architecture/workflow/sequence/dataflow/lifecycle diagrams and Mermaid conversion). Update with `git -C ~/.agents/mattpocock-skills pull`, and `git -C ~/.agents/archify pull` for archify. The previous local set (ponytail, no-mistakes, herdr, lavish, and the rest of `coding-agent/common/skills/`) is retired; sources remain in this repo and a snapshot sits in `~/.skills-backup-2026-08-27/`.
- Skills load from three roots, not one, and the registry is a snapshot taken at session start: `~/.jcode/skills/`, `~/.agents/skills/` (standalone CLIs: bb-cli, chrome-devtools-axi, gh-axi), and the claude-mem plugin's bundle under `~/.claude/plugins/marketplaces/thedotmack/plugin/skills/` (babysit, make-plan, wowerpoint, and friends). Removing a skill from only one root leaves it showing up from the others, and edits never appear until a new session starts.
