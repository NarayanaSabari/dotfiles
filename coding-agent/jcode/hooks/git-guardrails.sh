#!/bin/bash
# jcode pre_tool hook: block git operations that silently destroy uncommitted
# work or rewrite shared history. Deliberately does NOT block normal
# `git push` (this machine pushes early and often; no-mistakes is the push
# gate). Exit 0 = allow, exit 2 = block.
#
...
# arrives as an env var. Otherwise this mirrors
# coding-agent/claude/../hooks/git-guardrails.sh exactly - keep the two in
# sync.
