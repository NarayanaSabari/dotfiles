...
2. Run Codex non-interactively with a structured verdict:
   ...
   Pass `-m` and the effort explicitly. Without them codex falls back to `~/.codex/config.toml`, which is gitignored runtime state and currently sets `medium` - so the second opinion would think less hard than the routine no-mistakes gate, which forces `high`. Keep this in step with `agent_args_override.codex` in `.no-mistakes/config.yaml`.
