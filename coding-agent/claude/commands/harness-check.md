...
Most of what used to be in this command is now assertions. Run them first:

```
bash ~/dotfiles/coding-agent/verify.sh
```

Exit 0 means every mechanical check held: both harnesses' guards against both
payload shapes, fail-closed parsing, every hook path named in settings.json, the
CLAUDE.md to AGENTS.md import, and `~/AGENTS.md`. Non-zero names the failing
assertion. **Do not "fix" a failure by editing the assertion.**
...
3. **Settings sanity.** `~/.claude/settings.json` must be valid JSON. Flag any
...
4. **Agent frontmatter.** For each file in `coding-agent/claude/agents/`: the
   ...
   fable, inherit or a full model ID. Every entry in `tools` must survive the
   ...
   exempt. An agent whose prompt tells it to use a named skill must either list
   `Skill` in `tools` or preload it via `skills`, or the instruction is
   unfollowable.

5. **Anything in `dotfiles/.claude/` that is not a symlink.** That directory is
   only the stow shim; everything the agents read belongs in `coding-agent/`. A
   real file appearing there means something wrote outside the repo's structure.

Report a short table of what resolves and what does not. If something is broken,
