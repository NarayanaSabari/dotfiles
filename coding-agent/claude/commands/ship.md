...
7. **If this ran in a worktree, say so in the handoff.** The gate ends before the merge, so memory adoption cannot be part of it. Once the user merges, the order is: `git pull --ff-only` on the default branch, then `npx claude-mem adopt --branch <branch>` (passed explicitly - squash merges are not reported by `git branch --merged HEAD`), and only then remove the worktree. Removing it first strands that worktree's observations permanently. The `worktree-adopt-guard` hook blocks the removal if adoption is still pending, so this is a reminder rather than the safety net.
