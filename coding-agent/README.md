...
| `codex-worker` | sonnet | Drives `codex exec --sandbox workspace-write` so implementation bills against the ChatGPT subscription, not the Claude limit, then verifies the diff |
